...
rockSensorSD::rockSensorSD(G4String name)
...
void rockSensorSD::Initialize(G4HCofThisEvent* HCE)
{
  sensorCollection = new rockSensorHitsCollection
...
G4bool rockSensorSD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
  ...
  rockSensorHit* newHit = new rockSensorHit();
...
void rockSensorSD::EndOfEvent(G4HCofThisEvent* HCE)
{
  G4int NbHits = sensorCollection->entries();
  if (verboseLevel>0) {
  ...
  for (G4int i=0;i<NbHits;i++){
	  rockSensorHit* hit = (*sensorCollection)[i];
